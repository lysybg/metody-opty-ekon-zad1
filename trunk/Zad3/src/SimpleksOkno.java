...
public class SimpleksOkno extends javax.swing.JFrame {
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    public void UzupelnijTabele(){
		...
		//Uzupełniamy tab zmiennymi w jtextfietów które sa w tablicy tymczasowej
		//tablica x
		...
		//Uzupełniamy tab  1 lub -1 lub 0 w zależności od znaków i tego czy dany x występuje w równaniu
		...
		//Uzupełniamy "pionową" tablice z opisami zmiennych x3 x4 x5....
		...
		//Uzupełniamy wartości zmiennych bazwoych (kolumna Cb wykłady)
		for(int i=0; i<ograniczenia; i++){
		...
		//uzupełniamy tablice (kolumne pionową) Cb  (wykłady)
		for(int i = 0; i < ograniczenia; i++){
			Cb[i] = 0;
		}
		
		//uzupełniamy tablice Zj - Cj  wzorem : ci * xij  - cj    ci to u nas Cb czyli wartosci zmiennych bazowych a cj to wartosci funkcji celu
		for( int i = 0; i < ograniczenia; i++){
			for(int j =0; j < produkty; j++){
    ...
    // 1. Wyznaczenie rozwiązania wyjściowego spełniającego warunki optymalności
    /*     Ze wszystkich wyrazów wolnych wybieramy najmniejszy. Zmienna ta jest zmienną
     *     opuszczającą bazę. Jeżeli jest więcej, niż jedna najmniejsza wartość, wtedy
     *     wybieramy zmienną o najniższym numerze.
     */
    ...
    // 2. 
    /*     Kryterium wejscia
     *	   Obliczamy ilorazy wartości wskaźników optymalności przez odpowiadające im
     *     elementy wiersza dla zmiennej opuszczającej bazę ((zj - cj) : xij)
     *     dla tych elementów rozpatrywanego wiersza, które są ujemne.
     *     Do bazy wchodzi ta zmienna, dla której wartość bezwzględna odpowiadającego jej
     *     ilorazu jest najmniejsza.
     *     Jeżeli jest więcej niż jedna najmniejsza wartość tego ilorazu,
	 *     wtedy wybieramy zmienną o najniższym numerze.
     */
    //wyliczamy ilorazy
    for(int i = 0; i < produkty; i++){
    ...
    // 2. Czy rozwiązanie jest dopuszczalne?  Tak - koniec  Nie - idziemy dalej
    /*     Ze wszystkich wyrazów wolnych wybieramy najmniejszy. Zmienna ta jest zmienną
     *     opuszczającą bazę. Jeżeli jest więcej, niż jedna najmniejsza wartość, wtedy
     *     wybieramy zmienną o najniższym numerze.
     */
    ...
    // 3. Określenie indeksu wektora wyeliminowanego z bazy
    /*     Ze wszystkich wyrazów wolnych wybieramy najmniejszy. Zmienna ta jest zmienną
     *     opuszczającą bazę. Jeżeli jest więcej, niż jedna najmniejsza wartość, wtedy
     *     wybieramy zmienną o najniższym numerze.
     */
 ...
 /*   	
   ...
   //sprawdzamy kryterium optymalnosci
    //	while(KrteriumOptymalnosci){ 
    		...
    		//SPrawdzamy kryterium optymalnosci: zgodnie z teorią:
    		//Jeżeli wszystkie zmienne niebazowe występujące w funkcji celu mają 
    		//współ¬czynniki niedodatnie, to rozpatrywane rozwiązanie bazowe jest 
    		//rozwiązaniem optymalnym. W takim wypadku kończymy poszukiwania rozwiązania opty¬malnego. 
    		//if(CzyMax == false){
    ...
    //	if(KrteriumOptymalnosci){
    		...
    		for(int i=0; i<ograniczenia; i++){
    			if(tab[i][NumerMaxa] > 0){
    				...
    				Ilorazy[i] = -1;   // sygnał ze to pomijamy!!!!!
    			}
    		}
    		temp1=Ilorazy[0];
    		//Szukamy najmniejszego ilorazu
    ...
    public void WyswietlTabele(){
      ...
      //------------------TRZECI WIERSZ I RESZTA TABELI---------------------
        for(int i=0; i<ograniczenia; i++){
        	...
        	for(int k=0; k<ograniczenia+produkty; k++){
        		//zaokrąglamy do 2 miejsca po przecinku
    				a = tab[i][k];
    				a *= 100; 
        	        a = Math.round(a);
        	        a /= 100; 
      ...
      //------------------OSTATNIE DWA WIERSZE---------------------
    	...
    	//---------pionowe wyświetlanie x bazowych  x3 x4 x5 ....
    	jTextArea1.append("  ");
